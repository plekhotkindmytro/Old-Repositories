...
public class Http {
	...
	public static byte[] getRequest(String url) {
		try {
			...
			Map<String, List<String>> requestHeaders = conn
					.getRequestProperties();
			int code = conn.getResponseCode();

			if (code / 100 == 3) {
				System.out
						.println("Java didn't redirect automatically, going manual: "
			...
			if (code == 403) {
				...
				for (String k : requestHeaders.keySet()) {
					if (k != null) {
						List<String> vals = (List<String>) requestHeaders
								.get(k);
						for (String v : vals) {
							s = s + k + ": " + v + "\n";
						}
				...
				Map<String, List<String>> responseHeaders = conn
						.getHeaderFields();
				for (String k : responseHeaders.keySet()) {
					if (k != null) {
						List<String> vals = (List<String>) responseHeaders
								.get(k);
						for (String v : vals) {
							s = s + k + ": " + v + "\n";
						}
	...
	public static byte[] postRequest(String url, String postdata,
			String contentType) {
	...
	public static byte[] postRequest(String url, byte[] postdata,
			String contentType) {
		try {
			URL u = new URL(url);
	...
	public static void sendResponse(OutputStream out, String contentType,
			String data) {
	...
	public static String readUntil(InputStream is, byte[] endSequence) {
		...
		try {
			...
			System.out
					.println("readUntil unable to read from InputStream, endSeq: "
							+ new String(endSequence));
			e.printStackTrace();
		}
		try {
