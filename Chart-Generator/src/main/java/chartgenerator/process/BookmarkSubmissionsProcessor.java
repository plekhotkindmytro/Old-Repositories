...
public class BookmarkSubmissionsProcessor {

	private static final int MAX_PERCENTS = 100;

	private final BookmarkSubmissionInputs inputs;
	private final GlobalSettings globalSettings;
	private final List<BookmarkSubmissionOutput> outputs;
	...
	public BookmarkSubmissionsProcessor(final BookmarkSubmissionInputs inputs,
	...
	private List<BookmarkSubmissionOutput> generateOutputs() {
		...
		final List<String> sitePool = RandomSitePool.createPool(
				inputs.siteFrequencies, inputs.bookmarkSubmissionsNumber);

		final int digitsNumber = Integer.toString(
				inputs.bookmarkSubmissionsNumber).length();

		List<BookmarkSubmissionOutput> outputList = new ArrayList<BookmarkSubmissionOutput>();
		for (int i = 1; i <= inputs.bookmarkSubmissionsNumber; i++) {
			// KG name
			...
			if (randomKeywordGroupInputs != null) {
				...
				BookmarkSubmissionOutput output = new BookmarkSubmissionOutput(
	...
	public List<BookmarkSubmissionOutput> getOutputs() {
		return outputs;
	}
}
